...
class WhisperSpeechRecognitionService(TranscriptionService):
    def __init__(self) -> None:
        super().__init__()
        self.transcripted_audio_file_path: str | None = None
        ...
        def save_transcription_in_file(audio: speech_recognition.AudioData) -> None:
            try:
                with open(transcription_file, "a", encoding="utf-8") as f:
                    f.write(recognizer.recognize_whisper(audio))
                    f.write(" ")
            except speech_recognition.UnknownValueError:
                pass
...
TRANSCRIPTION_SERVICE_BACKENDS_MAPPER: dict[str, type[TranscriptionService]] = {
    ...
    TranscriptionServiceBackends.WHISPER_SPEECH_RECOGNITION.value: WhisperSpeechRecognitionService
}
